...
const TRANSCRIPTION_PROMPT = `Прослушай аудиозапись телефонного разговора и верни ТОЛЬКО JSON без markdown:
{
  ...
  "client_profile": "<структурированный профиль клиента в 3-5 предложениях: что ищет, параметры (площадь, комнатность), цель покупки (жильё/инвестиция), бюджет, локация, стадия готовности>",
  "communication_notes": "<особенности коммуникации: как лучше связаться, предпочтения, договорённости о следующем шаге, срочность>",
...
function formatTgReply(transcription, score) {
  ...
  const clientName = transcription.client_name ? ` — ${transcription.client_name}` : "";

  const lines = [
    `${gradeEmoji} <b>Оценка: ${score.score}/100 — ${score.grade}</b>  |  🕐 ${formatDuration(transcription.duration_seconds)}`,
    "",
    `<b>При заходе сказать, что звонил ваш личный ассистент Александр, передал, что${clientName}:</b>`,
    "",
  ];

  if (transcription.client_profile) {
    lines.push(transcription.client_profile);
    lines.push("");
  }

  if (transcription.communication_notes) {
    lines.push(`<b>Особенности коммуникации:</b>`);
    lines.push(transcription.communication_notes);
    lines.push("");
  }

  lines.push(`<b>Параметры запроса:</b>`);
  lines.push(`💰 Бюджет: ${score["бюджет"] || "—"}`);
  lines.push(`📍 Локация: ${score["локация"] || "—"}`);
  lines.push(`🛏 Комнатность: ${score["комнатность"] || "—"}`);
  lines.push(`📅 Срок покупки: ${score["срок_покупки"] || "—"}`);
  lines.push(`🏗 Тип жилья: ${score["тип_жилья"] || "—"}`);

  if (score.recommendation) {
    lines.push("");
    lines.push(`💡 <i>${score.recommendation}</i>`);
  }
