...
async function analyzeBuffer(audioBuffer, apiKey) {
  ...
  try {
    ...
    console.error("[transcribe] raw (first 500):", raw.slice(0, 500));
    throw new Error(`JSON parse failed: ${e.message}`);
  }
}

async function transcribeBuffer(audioBuffer, apiKey) {
  const { transcription } = await analyzeBuffer(audioBuffer, apiKey);
...
function formatTgReply(transcription, score, managerName) {
  const clientName = transcription.client_full_name || transcription.client_name || "клиент";
  ...
  if (transcription.client_phone) lines.push(`📞 ${transcription.client_phone}`);
  lines.push(`<i>🕐 ${formatDuration(transcription.duration_seconds)}</i>`);
  lines.push("");
  lines.push(`<pre>При заходе сказать, что звонил(а) ваш личный ассистент ${mgr}, передал(а), что:\n\n${transcription.summary || "—"}</pre>`);
  lines.push("");
  lines.push("─────────────────");
  lines.push("");
  lines.push(`<b>Оценка привлекательности лида для брокера: ${score.score}%</b>`);

  if (score.factors) lines.push(`Факторы соответствия: ${score.factors}`);
  if (score.limiting_factors) lines.push(`Ограничивающие факторы: ${score.limiting_factors}`);
