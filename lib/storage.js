...
async function setUser(chatId, data) {
  const sql = getDb();
  await sql`
    INSERT INTO users (chat_id, name, state, support_category, dm_target_id, dm_target_name)
    VALUES (${chatId}, ${data.name || null}, ${data.state || "awaiting_name"}, ${data.support_category || null}, ${data.dm_target_id || null}, ${data.dm_target_name || null})
